 ...
 <div class="note bg-boxBG border-t-4 border-infoBorder rounded-b px-4 py-2 shadow-md" role="alert">
   <div class="flex">
     <div class="py-1 inline-flex items-center flex-row">
      <i class="ri-information-2-line ri-1x text-infoIcon"></i><p class="font-sans font-semibold text-sm text-infoIcon">"Tree" and "graph" are used interchangeably due to the fact that all trees are inherently graphs since they don't allow cycles, but not all graphs are trees.</p>
    </div>
   </div>
 </div>
...
<div class="note bg-boxBG border-t-4 border-ideaBorder rounded-b px-4 py-2 shadow-md" role="alert">
  <div class="flex">
    <div class="py-1 inline-flex items-center flex-row">
     <i class="ri-lightbulb-line ri-1x text-ideaIcon"></i><p class="font-sans font-semibold text-sm text-ideaIcon content-center">Instead of removing nodes without dependencies one-by-one like you normally would, you can scan the entire queue and remove all source nodes at once to execute tasks in parallel.</p>
   </div>
  </div>
</div>
...
```java title="TopologicalSort.java" {11-13}
...
```java title="TopologicalSort.java" {12, 14-17, 19}
public class TopologicalSort {
    class Task {
        private int name;
        private int version;
...
```java title="TopologicalSort.java" {23-34}
public class TopologicalSort {
    class Task {
        private int name;
        private int version;
...
```java title="TopologicalSort.java" {29-31}
public class TopologicalSort {
    class Task {
        private int name;
        private int version;
